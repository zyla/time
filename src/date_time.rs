...
mod sealed {
    ...
    impl OffsetType for () {}
    impl OffsetType for UtcOffset {}

    pub trait MaybeOffset: Copy {
        // Statically restrict the type to be `()` and `UtcOffset`.
        type Offset: Copy + OffsetType;

        // True if and only if `Offset` is `UtcOffset`.
        const HAS_OFFSET: bool;

        fn as_offset(offset: Self::Offset) -> Option<UtcOffset>;
        #[cfg(feature = "parsing")]
        fn try_from_parsed(parsed: Parsed) -> Result<Self::Offset, error::TryFromParsed>;
        #[cfg(feature = "parsing")]
        fn from_offset(offset: UtcOffset) -> Self::Offset;
    }
}

impl MaybeOffset for maybe_offset::None {
    type Offset = ();
    ...
    #[cfg(feature = "parsing")]
    fn from_offset(_: UtcOffset) -> Self::Offset {}
}

impl MaybeOffset for maybe_offset::Fixed {
    type Offset = UtcOffset;
...
const fn maybe_offset_as_offset<O: MaybeOffset>(offset: O::Offset) -> Option<UtcOffset> {
    if O::HAS_OFFSET {
...
pub struct DateTime<O: MaybeOffset> {
    pub(crate) date: Date,
    pub(crate) time: Time,
    pub(crate) offset: O::Offset,
}

// region: constructors
impl DateTime<maybe_offset::None> {
...
impl DateTime<maybe_offset::Fixed> {
    pub const fn from_unix_timestamp(timestamp: i64) -> Result<Self, error::ComponentRange> {
        ...
        Ok(DateTime::new(date, time).assume_utc())
    }
...
impl DateTime<maybe_offset::None> {
    ...
    pub const fn assume_offset(self, offset: UtcOffset) -> DateTime<maybe_offset::Fixed> {
        DateTime {
            date: self.date,
            time: self.time,
            offset,
    ...
    pub const fn assume_utc(self) -> DateTime<maybe_offset::Fixed> {
...
impl<O: MaybeOffset> DateTime<O> {
    ...
    pub(crate) const fn to_offset_raw(self, offset: UtcOffset) -> (i32, u16, Time) {
        let from = match maybe_offset_as_offset::<O>(self.offset) {
            Some(offset) => offset,
            None => {
...
impl DateTime<maybe_offset::Fixed> {
    pub const fn replace_date_time(self, date_time: DateTime<maybe_offset::None>) -> Self {
...
#[cfg(feature = "std")]
impl Sub<SystemTime> for DateTime<maybe_offset::Fixed> {
    type Output = Duration;
...
#[cfg(feature = "std")]
impl Sub<DateTime<maybe_offset::Fixed>> for SystemTime {
    type Output = Duration;

    fn sub(self, rhs: DateTime<maybe_offset::Fixed>) -> Self::Output {
        DateTime::<maybe_offset::Fixed>::from(self) - rhs
    }
}

#[cfg(feature = "std")]
impl PartialEq<SystemTime> for DateTime<maybe_offset::Fixed> {
...
#[cfg(feature = "std")]
impl PartialEq<DateTime<maybe_offset::Fixed>> for SystemTime {
    fn eq(&self, rhs: &DateTime<maybe_offset::Fixed>) -> bool {
        &DateTime::<maybe_offset::Fixed>::from(*self) == rhs
    }
}

#[cfg(feature = "std")]
impl PartialOrd<SystemTime> for DateTime<maybe_offset::Fixed> {
...
#[cfg(feature = "std")]
impl PartialOrd<DateTime<maybe_offset::Fixed>> for SystemTime {
    fn partial_cmp(&self, other: &DateTime<maybe_offset::Fixed>) -> Option<Ordering> {
        DateTime::<maybe_offset::Fixed>::from(*self).partial_cmp(other)
    }
}

#[cfg(feature = "std")]
impl From<SystemTime> for DateTime<maybe_offset::Fixed> {
...
#[cfg(feature = "std")]
impl From<DateTime<maybe_offset::Fixed>> for SystemTime {
    fn from(datetime: DateTime<maybe_offset::Fixed>) -> Self {
        let duration = datetime - DateTime::<maybe_offset::Fixed>::UNIX_EPOCH;
...
impl From<js_sys::Date> for DateTime<maybe_offset::Fixed> {
...
impl From<DateTime<maybe_offset::Fixed>> for js_sys::Date {
    fn from(datetime: DateTime<maybe_offset::Fixed>) -> Self {
